...
def build_transforms() -> Tuple[transforms.Compose, transforms.Compose, transforms.Compose]:
    ...
    train_tf = transforms.Compose(
        [
            ...
            transforms.ColorJitter(brightness=0.35, contrast=0.35, saturation=0.3, hue=0.1),
            transforms.RandomApply([transforms.GaussianBlur(kernel_size=3)], p=0.25),
            transforms.RandomAffine(degrees=0, translate=(0.12, 0.12), scale=(0.85, 1.15), shear=10),
            transforms.ToTensor(),
...
def main() -> None:
    ...
    model = build_model(num_classes=len(class_names), pretrained=True, dropout=0.3).to(device)
    ...
    for epoch in range(1, NUM_EPOCHS + 1):
        ...
        if val_acc > best_val_acc:
            ...
            print(f"🔥 New BEST model: {best_val_acc:.4f}")
    ...
    plot_path = ARTIFACTS_DIR / "train_val_curves.png"
    plot_curves(history, plot_path)

    wandb.log({
        "curves": wandb.Image(str(plot_path))
    })
